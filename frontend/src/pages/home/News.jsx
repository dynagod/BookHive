...
const News = () => {
  return (
    <div className='py-16 px-4 bg-gradient-to-r from-[#e2f0ff] via-[#dcebff] to-[#c5e1ff]'>
        <h2 className='text-[#1A1A1A] text-3xl font-semibold mb-6'>News</h2>
        ...
        
        {
            news.map((item, index) => (
                <SwiperSlide key={index}>
                    <div className='flex flex-col sm:flex-row sm:justify-between items-center gap-12 p-6 bg-gradient-to-r from-white via-[#fff5f1] to-[#ffeae0] rounded-lg shadow-md border border-[#ffd6cc]'>
                        {/* content */}
                        <div className='py-4'>
                            <Link to="/">
                                 <h3 className='text-[#1A1A1A] text-lg font-medium hover:text-[#00214D] mb-4'>{item.title}</h3>
                            </Link>
                            <div className='w-12 h-[4px] bg-[#00214D] mb-5'></div>
                            <p className='text-[#5E5E5E] text-sm'>{item.description}</p>
                        </div>
